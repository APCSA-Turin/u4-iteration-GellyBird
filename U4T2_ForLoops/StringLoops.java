...
public class StringLoops {
    ...
    public int countCharacters(String character, String searchString) {
        ...
        for (int i = 0; i < searchString.length(); i++) {
            if (searchString.substring(i,i+1).equals(character.toLowerCase()) || searchString.substring(i,i+1).equals(character.toUpperCase())) {
                total ++;
            }
        }
      return total;
    ...
    */  
    public int countString(String searchString, String origString) {
      ...
      for(int i = 0; i < origString.length(); i++) {
        ...
        if (i + searchLength - 1 < origString.length()) {
          if((origString.substring(i,i+searchLength).equals(searchString))) {
            total++;
          }
        }
 ...
 /* Returns true if myString is a palindrome, i.e. the characters read the same
    ...
    */
    public boolean isPalindrome(String myString) {
      ...
      reversedString = removeString(" ", reversedString);
      myString = removeString(" ", myString);
      if (reversedString.contains(myString)) {
        return true;
      } else {
        return false;
      }
